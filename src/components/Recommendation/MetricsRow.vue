  ...
  <div class="grid grid-cols-3 gap-6 max-[900px]:grid-cols-1">
    ...
    <div class="bg-surface rounded-xl p-6 shadow-sm border border-border flex flex-col">
      <div class="flex justify-between items-center mb-2">
        <span class="text-[0.65rem] font-extrabold text-text-muted tracking-[1px]">{{ t('recommendation.hydration') }}</span>
        <div class="w-8 h-8 rounded-full flex items-center justify-center bg-[#e6f6ef] text-success-green">
          <Droplets width="16" height="16" />
        </div>
      </div>
      <h3 class="text-[1.1rem] font-extrabold text-text-main mb-4">{{ t('recommendation.soilMoisture') }}</h3>
      ...
      <p class="text-[0.75rem] m-0 font-medium"
        :class="weather.soilMoisture < 25 ? 'text-red-500' : 'text-success-green'">
        {{ weather.soilMoisture < 25 ? t('recommendation.soilCritical') : t('recommendation.soilAdequate') }}
      </p>
    </div>
    ...
    <div class="bg-surface rounded-xl p-6 shadow-sm border border-[rgba(251,146,60,0.3)] shadow-[0_4px_12px_rgba(251,146,60,0.05)] flex flex-col">
      <div class="flex justify-between items-center mb-2">
        <span class="text-[0.65rem] font-extrabold text-text-muted tracking-[1px]">{{ t('recommendation.exposure') }}</span>
        <div class="w-8 h-8 rounded-full flex items-center justify-center bg-[#fff4e5] text-[#f59e0b]">
          <Sun width="16" height="16" />
        </div>
      </div>
      <h3 class="text-[1.1rem] font-extrabold text-text-main mb-4">{{ t('recommendation.uvIndex') }}</h3>
      <div class="flex items-center gap-3 mb-6 flex-1">
      ...
      <p class="text-[0.75rem] text-text-muted m-0 font-medium">{{ t('recommendation.uvPeak') }}</p>
    </div>

    <!-- Humidity -->
    <div class="bg-surface rounded-xl p-6 shadow-sm border border-border flex flex-col">
      <div class="flex justify-between items-center mb-2">
        <span class="text-[0.65rem] font-extrabold text-text-muted tracking-[1px]">{{ t('recommendation.atmosphere') }}</span>
        <div class="w-8 h-8 rounded-full flex items-center justify-center bg-[#ebf5ff] text-[#3b82f6]">
          <Droplet width="16" height="16" />
        </div>
      </div>
      <h3 class="text-[1.1rem] font-extrabold text-text-main mb-4">{{ t('recommendation.humidity') }}</h3>
      <div class="flex items-center gap-3 mb-6 flex-1">
      ...
      <p class="text-[0.75rem] text-success-green m-0 font-medium">{{ t('recommendation.humidityStable') }}</p>
    </div>
  </div>
</template>
