...
import { Droplet, SunLight, DropletHalf, Wind } from '@iconoir/vue'
...
const metrics = computed(() => [
  ...
  },
  {
    ...
    icon: markRaw(SunLight),
  },
  {
    label: 'SOIL MOISTURE',
    value: `${weather.soilMoisture}%`,
    icon: markRaw(DropletHalf),
  },
  {
    label: 'WIND SPEED',
  ...
  <div class="bg-surface rounded-xl p-8 shadow-sm mb-8">
    ...
    <div class="flex items-center gap-6 mb-3">
      <div class="flex items-start">
        <span class="text-[6.5rem] max-lg:text-[4.5rem] font-extrabold leading-[0.9] tracking-[-3px] text-text-main">{{ weather.temp }}°</span>
        <span class="text-[2.5rem] max-lg:text-[1.8rem] font-semibold text-text-main mt-2">F</span>
      </div>
      <SunLight class="max-lg:w-10 max-lg:h-10 shrink-0" width="64" height="64" color="#fbbd06" />
    </div>
    ...
    <div class="grid grid-cols-4 max-lg:grid-cols-2 gap-4">
      <div v-for="metric in metrics" :key="metric.label"
        ...
        <div class="w-8 h-8 rounded-full bg-success-green flex items-center justify-center text-white">
